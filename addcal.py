...
def adiciona(nome, desc, dataini):
    ...
    # time.
    if os.path.exists(Path().resolve().parent / 'credenciais'/ 'token.pickle'):
        with open(Path().resolve().parent / 'credenciais'/ 'token.pickle', 'rb') as token:
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            flow = InstalledAppFlow.from_client_secrets_file(Path().resolve().parent / 'credenciais'/ 'CMPcal.json', SCOPES)
            creds = flow.run_local_server(port=0)
        # Save the credentials for the next run
        with open(Path().resolve().parent / 'credenciais'/ 'token.pickle', 'wb') as token:
